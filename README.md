...
│   ├── lesson-form.php        # Add/Edit lesson with audio upload
│   ├── access-codes.php       # Access code management
│   ├── users.php              # User management
│   └── activity-logs.php      # Activity logs
│
├── api/                        # API endpoints
│   ├── get-audio-token.php    # Generate audio streaming token
│   └── save-progress.php      # Save user progress
│
...
│   ├── AudioStream.php        # Audio streaming handler
...
├── lessons.php                 # Lessons with audio player
...
### Download Prevention
- ❌ No direct file access (protected by .htaccess)
- ❌ Right-click disabled
- ❌ Keyboard shortcuts for save/download blocked
- ❌ Audio controlsList set to 'nodownload'

### Streaming Security
- ✅ Token-based audio URLs
- ✅ Short-lived tokens (5 minutes default)
- ✅ Tokens bound to user, session, and IP
- ✅ Single-use tokens
- ✅ Server-side streaming (no direct file exposure)

### Recording Prevention (Best Effort)
- ❌ DevTools detection with audio pause
- ❌ MediaRecorder API blocked
- ❌ Web Audio createMediaStreamDestination blocked
- ❌ Print/screenshot protection (limited)
- ❌ Tab visibility detection (pause when hidden)
- ⚠️ Dynamic watermarks with user info (forensic tracking)
